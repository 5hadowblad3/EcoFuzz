...
static void schedule(void) {
    ...
    while (q) {
        if (q->exec_by_mutation * p->mutation_num * get_sqrt(p->serial) < p->exec_by_mutation * q->mutation_num * get_sqrt(q->serial) &&
            q->state == 0) {
            p = q;
            current_entry = i;
        }
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
    ...
    fprintf(plot_file,
            "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f, %llu, %u, %lld, %lld, %lld\n",
            get_cur_time() / 1000, queue_cycle - 1, current_entry, queued_paths,
            pending_not_fuzzed, pending_favored, bitmap_cvg, unique_crashes,
            unique_hangs, max_depth, eps, total_execs, state_of_fuzz,
...
/*
      if (perf_score <= HAVOC_MAX_MULT * 100) {
        stage_max += temp_stage;
        perf_score += temp_score;
      }
*/
